...
class MonthlyCostCalculator(AbstractCost):
    def __init__(self, gas_modulated: Optional[bool] = False, name: Optional[str] = CostType.Monthly, **kwargs):
        super().__init__(gas_modulated=gas_modulated)
        self.monthly_cost = kwargs
        self.name = name

    def _compute_cost(self, n_expedition_by_month: int = N_EXPEDITION, *args, **kwargs) -> float:
