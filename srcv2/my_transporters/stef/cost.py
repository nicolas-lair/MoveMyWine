...
class StefCostByBottleCalculator(AbstractCost):
    ...
    def get_tarif_conditions(self, n_bottles: int) -> tuple[UnitType, TarifType, str]:
        ...
        tarif_type, tarif_id = tarif_info.loc[tarif_unit, [TarifStructureFile.Cols.type_, TarifStructureFile.Cols.tarif_id]]
    ...
    def _compute_cost_nationwide(self, expedition: MultiRefExpedition) -> pd.Series:
        ...
        tarif_unit, tarif_type, tarif_id = self.get_tarif_conditions(n_bottles=n_bottles_eq)
...
class StefTotalCost(TotalCostCalculator):
    def __init__(self):
        super().__init__({
            ...
            CostType.Expedition: FixedCostByExpe(position_cost=tp.position_cost, gas_modulated=True),
            CostType.Security: FixedCostByExpe(security_cost=tp.security_cost, gas_modulated=False)
        })
