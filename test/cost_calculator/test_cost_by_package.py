...
class TestCostByPackageCalculator:
    ...
    cost_with_gas_mod = CostByPackageCalculator(
        gas_modulated=True,
        costs_by_package=base_package_cost,
        extra_package_costs=custom_extra_package_cost,
    )
    cost_without_gas_mod = CostByPackageCalculator(
        gas_modulated=False,
        costs_by_package=base_package_cost,
        extra_package_costs=custom_extra_package_cost,
    )
    ...
    def test_base_cost(self):
        assert (
            self.cost_without_gas_mod.base_cost_by_package
            == base_package_cost["package_cost"]
        )
    ...
    def test_cost_without_gas_mod(self):
        assert (
            self.cost_without_gas_mod.compute_cost(SingleRefExpedition(n_bottles=12))
            == 2 * base_package_cost["package_cost"]
        )
        assert (
            self.cost_without_gas_mod.compute_cost(SingleRefExpedition(n_bottles=24))
            == 4 * base_package_cost["package_cost"] + 1
        )
        assert (
            self.cost_without_gas_mod.compute_cost(SingleRefExpedition(n_bottles=240))
            == 40 * base_package_cost["package_cost"] + 3.5
        )

    def test_cost_with_gas_mod(self):
        assert self.cost_with_gas_mod.compute_cost(
