...
class TestSingleRefExpedition:
    ...
    expe_with_package = SingleRefExpedition(
        bottle_type=BaseBottle(empty_weight=0.5, volume=0.75),
        n_bottles=3,
        package=package,
    )
    other_expe_with_package = SingleRefExpedition(
        bottle_type=BaseBottle(empty_weight=1, volume=1.5),
        n_bottles=12,
        package=package,
    )
    ...
    def test_n_package(self):
        ...
        assert self.expe_with_package.n_packages == 1
        assert self.other_expe_with_package.n_packages == 2
...
class TestMultiRefExpedition:
    ...
    def test_n_packages(self):
        ...
        assert (
            expe.n_packages == self.first_expe.n_packages + self.second_expe.n_packages
        )
    ...
    def test_bottle_eq(self):
        expe = MultiRefExpedition([self.first_expe, self.second_expe])
        assert (
            expe.n_bottles_equivalent
            == self.first_expe.n_bottles_equivalent
            + self.second_expe.n_bottles_equivalent
        )
