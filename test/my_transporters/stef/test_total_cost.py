...
class TestTotalCost:
    ...
    def test_compute_cost(self):
        ...
        dep = "75"
        assert self.cost_calculator.compute_cost(gas_price=1., expedition=exp, department=dep) == 43.14+0.7
        assert self.cost_calculator.compute_cost(gas_price=1.45, expedition=exp, department=dep) == 49.02
        ...
        dep = "69"
        assert self.cost_calculator.compute_cost(gas_price=1., expedition=exp, department=dep) == round(56.31+0.7, 2)
        assert self.cost_calculator.compute_cost(gas_price=1.4, expedition=exp, department=dep) == 63.2
